...
function handleLimitedCapacity(): boolean {
    ...
    const isLimited = 
        (bodyText.includes('submissions at a time') && bodyText.includes('currently full')) ||
...
function extractStudyDataFromCard(card: Element) {
    try {
...
function updateIndicator(type: 'ok' | 'warn' | 'error', text: string) {
